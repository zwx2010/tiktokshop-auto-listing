...
_STAGE_TEMPLATES = {
    ...
    "interpret": {
        ...
        "prompt": (
            ...
            "   含'采集'(且不是'从采集库选...')→capture_db(采集入库到选品库,不做制表/审图/上架);\n"
            "   含'只审/不传/别上架/先审'→review_only;含'只采/先采'→capture_only;\n"
            "   含'选N件品类'或'上架N个品类'且带目标站点(如'选3件耳环到TH'/'上架三个耳环到ph'/"
            "'从采集库选10件手链到PH'/'上架ph站点3件耳环'/'上架到ph站点3件耳环'/'上架泰文站点3件耳环')"
            "→select_upload(从采集库按品类选N件生成上架任务,不采集);站点词在数量前或带'到'都算;\n"
            "   含'上架/选N件'且带目标站点但没写品类(如'上架ph站点3件'/'上架10件到TH')也→select_upload"
            "(从采集库选N件,品类不限,不采集;用户定规:上架≠采集,没提采集就不重新采集);\n"
            "   含'上架/选N件'但没写站点也没写品类(如'上架3件'/'上架3件耳环')同样→select_upload,"
            "站点默认 ph(用户定规:没提采集就不重新采集,从采集库选N件);纯'上架'没数量不适用;\n"
            "   含'一批/一些/几个'等模糊数量(如'上架一批商品'/'上架一批耳环')→select_upload,"
            "target 默认 10,站点默认 ph,从采集库选(用户定规);\n"
    ...
    "upload": {
        ...
        "prompt": (
            ...
            "-ProductLine accessories -MarketCode {market_upper} -WorkbookPath \"{tables}\" "
            "-Import -ImportTimeoutSeconds 600 2>&1。"
            "第一条确保 TikTok 卖家浏览器(端口 9344)已启动,已在跑就秒退,正常。"
            "第二条在卖家后台 CDP 真实上传,需登录态,属正常,你的权限已全部放行。"
            "注意:批量导入 10+ 款时 TikTok 后台解析+导入要几分钟,ImportTimeoutSeconds 已给到 600s,别提前判失败。"
